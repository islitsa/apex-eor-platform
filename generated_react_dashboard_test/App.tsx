...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    data.forEach((record: any) => {
      if (record.StateName) states.add(record.StateName);
      if (record.OperatorName) operators.add(record.OperatorName);
      if (record.CountyName) counties.add(record.CountyName);
    });
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((record: any) =>
      record.StateName?.toLowerCase().includes(term) ||
      record.OperatorName?.toLowerCase().includes(term) ||
      record.WellName?.toLowerCase().includes(term) ||
      record.CountyName?.toLowerCase().includes(term)
    );
  }, [data, searchTerm]);
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-blue-800 space-y-1">
                  ...
                  <p><strong>Columns:</strong> {info.columns?.join(', ') || 'Loading...'}</p>
                  <p><strong>Pipeline Stages:</strong> downloads → extracted → parsed</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <StatsCard
              icon="map"
              label="States"
              value={stats.uniqueStates.toLocaleString()}
              color="green"
            />
            <StatsCard
              icon="business"
            ...
            <StatsCard
              icon="location_city"
              label="Counties"
              value={stats.uniqueCounties.toLocaleString()}
              color="orange"
            />
          </div>
        )}
