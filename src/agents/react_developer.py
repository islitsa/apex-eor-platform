...
# Lazy-loaded Pinecone KB for interaction patterns
_design_kb_instance = None

def _get_design_kb():
    """Lazy-load DesignKnowledgeBasePinecone to avoid import overhead"""
    global _design_kb_instance
    if _design_kb_instance is None:
        try:
            from src.knowledge.design_kb_pinecone import DesignKnowledgeBasePinecone
            _design_kb_instance = DesignKnowledgeBasePinecone()
            # Ensure patterns are loaded
            _design_kb_instance.ensure_interaction_patterns_loaded()
        except Exception as e:
            print(f"[React Developer] Warning: Could not load Pinecone KB: {e}")
            _design_kb_instance = None
    return _design_kb_instance
...
class ReactDeveloperAgent(AutonomousReactMixin):
    ...
    def _load_data_hooks_template(self) -> str:
        """
        Load the CANONICAL dataHooks.tsx template.

        This template is battle-tested and should NOT be regenerated by the LLM.
        It contains the correct:
        - API endpoints (/api/pipelines/{id}/files/preview)
        - Parameter names (page_size, not limit)
        - Response property names (totalRows camelCase)
        - Type definitions matching the actual API

        Returns:
            Content of dataHooks.tsx template, or empty string if not found
        """
        # Try new canonical template location first
        template_path = Path(__file__).parent / "templates" / "dataHooks.tsx"
        if template_path.exists():
            print(f"  [React Developer] Using CANONICAL dataHooks.tsx template")
            return template_path.read_text(encoding='utf-8')

        # Fallback to old location
        old_path = Path(__file__).parent.parent / "templates" / "data_hooks.tsx"
        if old_path.exists():
            return old_path.read_text(encoding='utf-8')

        print(f"  [React Developer] WARNING: No dataHooks template found!")
        return ""  # Fallback if template not found

    def _get_api_contract(self) -> str:
        """
        Get the API contract documentation for the React developer prompt.

        First tries to fetch from OpenAPI schema (/openapi.json).
        Falls back to hardcoded contract if API is not running.

        Returns:
            Formatted API contract string for injection into prompt
        """
        import requests

        # Try to fetch live OpenAPI schema
        try:
            response = requests.get("http://localhost:8000/openapi.json", timeout=2)
            if response.ok:
                openapi = response.json()
                # Extract relevant endpoints (simplified for now)
                print("  [React Developer] Loaded API contract from OpenAPI schema")
        except Exception:
            pass  # Fall through to hardcoded contract

        # Hardcoded contract (always reliable)
        return """
═══════════════════════════════════════════════════════
CRITICAL: BACKEND API CONTRACT (DO NOT DEVIATE)
═══════════════════════════════════════════════════════

ENDPOINTS:

1. GET /api/pipelines
   Returns: { success: boolean, pipelines: Pipeline[], summary: {...} }

2. GET /api/pipelines/{pipeline_id}/files/preview
   ⚠️ NOTE: Path is /files/preview (NOT /preview, NOT /data)

   Query Parameters:
   - file_path: string (required) - relative path like "Extracted/file.csv"
   - page: number (default: 1) - page number (1-indexed)
   - page_size: number (default: 100) - rows per page
   ⚠️ NOTE: Parameter is "page_size" (NOT "limit")

   Response (camelCase!):
   {
     columns: string[],
     rows: Record<string, unknown>[],
     totalRows: number,    // ⚠️ camelCase, NOT total_rows
     page: number,
     pageSize: number      // ⚠️ camelCase, NOT page_size
   }

HOOKS (from canonical dataHooks.tsx):

1. usePipelines()
   - Calls: GET /api/pipelines
   - Returns: { data: PipelinesResponse | null, loading, error }

2. useFilePreview(pipelineId, filePath, page, pageSize)
   - Calls: GET /api/pipelines/{id}/files/preview?file_path=X&page=Y&page_size=Z
   - Returns: { data: FilePreviewResponse | null, loading, error }
   - ⚠️ Response uses camelCase: data.totalRows (NOT data.total_rows)

INTERACTION PATTERN (File Explorer → Preview):

1. FileExplorerTree MUST have:
   - onFileSelect?: (filePath: string) => void
   - selectedFile?: string | null

2. Parent component (DatasetOverviewCard) MUST:
   - Track selectedFile state
   - Pass handleFileSelect to FileExplorerTree
   - Call useFilePreview(pipelineId, selectedFile, page, pageSize)
   - Display data.columns and data.rows when loaded

Example wiring:
```tsx
const [selectedFile, setSelectedFile] = useState<string | null>(null);
const { data, loading, error } = useFilePreview(pipelineId, selectedFile, 1, 20);
...
{selectedFile && data && (
  <table>
    <thead>{data.columns.map(c => <th>{c}</th>)}</thead>
    <tbody>{data.rows.map(r => <tr>...</tr>)}</tbody>
  </table>
)}
```
"""
    ...
    def build(self, design_spec: DesignSpec, context: Dict[str, Any]) -> Dict[str, str]:
        ...
        # Use v3 compressed prompt (~3k tokens vs ~12k original)
        prompt = self._create_react_prompt_v3(design_spec, data_sources, user_prompt, data_context, gradient_context)
...
{self._build_interaction_model_guidance(design_spec)}

{self._query_pinecone_patterns(design_spec)}
...
"""
    ...
    def _create_react_prompt_v3(self, design_spec: DesignSpec, data_sources: Dict, user_prompt: str, data_context: Dict = None, gradient_context: Dict = None) -> str:
        """
        v3 COMPRESSED React Developer prompt (~3k tokens vs ~12k original)

        70% smaller while preserving:
        - Strict API-only data fetching (NO mock data)
        - Type coercion requirements
        - File schema normalization
        ...
        # Build discovered sources JSON
        discovered_source_names = list(data_sources.keys())
        discovered_sources_json = json.dumps(discovered_source_names)

        # Build sources info (compact)
        sources_info_lines = []
        for name, info in data_sources.items():
            row_count = info.get('row_count', info.get('records', 0))
            status = info.get('status', 'unknown')
            sources_info_lines.append(f"- {name}: {row_count:,} records, status={status}")
        sources_info = "\n".join(sources_info_lines)
        ...
        if hasattr(design_spec, 'to_implementation_guidance'):
            ...
            design_summary = str(getattr(design_spec, 'design_reasoning', ''))[:400]

        # Component names for enforcement
        required_components = []
        if hasattr(design_spec, 'components') and design_spec.components:
            required_components = [
                comp.get('name') if isinstance(comp, dict) else getattr(comp, 'name', None)
            ...
            required_components = [n for n in required_components if n]

        components_list = ", ".join(required_components) if required_components else "per design spec"
        ...
        if data_context.get('success'):
            pipelines = data_context.get('pipelines', [])
            summary = data_context.get('summary', {})
            real_data_status = f"API LIVE: {len(pipelines)} pipelines, {summary.get('total_records', 0):,} records"
        else:
            real_data_status = f"API unavailable: {data_context.get('error', 'unknown')}"

        # Gradient hints (compact)
        gradient_hint = ""
        if gradient_context.get('domain_signals'):
            domain = gradient_context['domain_signals'].get('domain', 'generic')
            structure = gradient_context['domain_signals'].get('structure', 'flat')
            gradient_hint = f"Domain: {domain}, Structure: {structure}"

        prompt = f"""You are a React + TypeScript expert. Generate production-ready code.
...
DATA SOURCES:
{sources_info}
DISCOVERED_SOURCES = {discovered_sources_json}
{gradient_hint}
...
2. API SCHEMA (exact fields only)
   Pipeline: id, name, display_name, status
   Pipeline.metrics: file_count, record_count, data_size
   Pipeline.stages: [{{name, status}}]
   Pipeline.files: FileNode[]

   ❌ pipeline.total_records (doesn't exist)
   ❌ pipeline.fileCount (doesn't exist)
   ✅ pipeline.metrics.record_count

3. RESPONSE STRUCTURE
   usePipelines() returns: {{data: {{pipelines: Pipeline[], summary}}, loading, error}}

   ❌ data.filter(...) — data is NOT array
   ✅ data.pipelines.filter(...)

4. DISCOVERY FILTERING (mandatory)
   const DISCOVERED_SOURCES = {discovered_sources_json};
   ...
   // Use filtered everywhere, NOT data.pipelines

5. TYPE COERCION (prevents TypeError)
   ✅ String(value || '').replace(...)
   ✅ Number(value || 0).toFixed(2)
   ✅ pipeline?.stages?.map(...) ?? []
   ❌ value.replace(...) — crashes if value is number/null

6. FILE NORMALIZATION (for FileExplorer components)
   API returns two formats. Always normalize:
   ...
   const normalizeNode = (node: any): FileNode => {{
     ...
     if (node.children) {{
       ...
       if (node.subdirs) Object.values(node.subdirs).forEach(d => children.push(normalizeNode(d)));
       if (node.files) node.files.forEach(f => children.push({{...f, type:'file', children:[]}}));
     }}
     return {{
       name: node.name, path: node.path,
       type: node.type === 'folder' ? 'directory' : (node.type || 'directory'),
       children, file_count: node.file_count, size: node.size_bytes || node.size
     }};
   }};

   // Usage: pipeline.files.map(normalizeNode)

{self._get_api_contract()}

═══════════════════════════════════════════════════════
TECHNICAL SPEC
═══════════════════════════════════════════════════════

Stack: React 18+, TypeScript, Tailwind CSS, Material Symbols Rounded
Icons: <span className="material-symbols-rounded">icon_name</span>
State: useState, useMemo, useCallback (no class components)
Imports: Include .tsx/.ts extensions (Vite requirement)

7. IMPORT RULES (prevents TS6133 errors)
   ❌ import React from 'react';  // NOT NEEDED in React 18+
   ✅ import {{ useState, useMemo }} from 'react';  // Import only what you use

   Only import React if using React.createElement, React.Fragment, etc. directly.

8. EXPORT CONVENTION (prevents TS2613 errors)
   Every component MUST have BOTH exports:
   ✅ function MyComponent() {{ ... }}
      export {{ MyComponent }};
      export default MyComponent;

   ❌ export function MyComponent() {{ ... }}  // Missing default export

9. CALLBACK TYPE ANNOTATIONS (prevents TS7006 errors)
   ✅ data.map((item: Pipeline, idx: number) => ...)
   ✅ columns.map((col: string, idx: number) => ...)
   ❌ data.map((item, idx) => ...)  // Implicit any

10. INTERFACE CONSISTENCY (prevents TS2322 errors)
    Before passing props to a component, verify they match its interface.
    Only pass props that are DEFINED in the component's Props interface.

Component naming: Match design spec exactly
  "Hero Metrics Card" → HeroMetricsCard.tsx

File markers (for parsing):
  // === FILE: App.tsx ===
  // === FILE: types.ts ===
  // === FILE: components/Name.tsx ===

Required files: App.tsx, types.ts, dataHooks.tsx, index.css, package.json
Optional: components/*.tsx based on design spec
...
Generate complete, runnable code. Use // === FILE: name === markers.
No markdown fences. Raw code only.
Handle loading/error states. Fetch from http://localhost:8000/api/pipelines.
Filter to DISCOVERED_SOURCES only."""
    ...
    def _build_interaction_model_guidance(self, design_spec: 'DesignSpec') -> str:
        """
        Build INTERACTION MODEL guidance from DesignSpec.interaction_model

        This section teaches React Developer how to implement behavioral state machines
        from the UX Designer's interaction_model.

        Args:
            design_spec: Design specification containing interaction_model

        Returns:
            Formatted guidance section for the prompt
        """
        if not hasattr(design_spec, 'interaction_model') or not design_spec.interaction_model:
            return ""

        interaction_model = design_spec.interaction_model

        lines = []
        lines.append("")
        lines.append("=" * 80)
        lines.append("🔄 INTERACTION MODEL - BEHAVIORAL STATE MACHINES (CRITICAL)")
        lines.append("=" * 80)
        lines.append("")
        lines.append("The UX Designer has provided an interaction_model that defines HOW components")
        lines.append("must behave. YOU MUST IMPLEMENT THESE BEHAVIORS EXACTLY.")
        lines.append("")
        lines.append("ABSOLUTE RULES:")
        lines.append("1. NEVER GENERATE MOCK OR STATIC DATA ARRAYS")
        lines.append("   - If you're building a table, chart, or preview component, you MUST fetch")
        lines.append("     data via a hook or API function derived from interaction_model.data_fetch")
        lines.append("   - If you cannot find a suitable interaction, STOP and surface an error in")
        lines.append("     comments instead of using mock data")
        lines.append("")
        lines.append("2. MAPPING EVENTS TO COMPONENTS:")
        lines.append("   - onFileSelect → File explorer + preview components")
        lines.append("   - onPageChange → Pagination controls + data tables")
        lines.append("   - onDataFetchStart / onDataFetchError / onDataEmpty → loading/error/empty states")
        lines.append("   - Use component names and props (selectedFile, currentPage) to determine which")
        lines.append("     interaction applies")
        lines.append("")
        lines.append("3. USING react_pattern:")
        lines.append("   - If react_pattern is present, treat it as the canonical pattern for the hook call")
        lines.append("   - If the hook (e.g., useFilePreview) does NOT exist, you MUST:")
        lines.append("     a) Implement it in dataHooks.tsx calling the endpoint from data_fetch.endpoint")
        lines.append("     b) Import and use that hook in the component")
        lines.append("")
        lines.append("4. DEPENDENCY RULES:")
        lines.append("   - Translate dependency_rules into useEffect dependency arrays")
        lines.append("   - Example: 'refetch when filePath changes' → include filePath in deps")
        lines.append("")
        lines.append("5. STATE CHANGES:")
        lines.append("   - state_changes tell you what React state must exist and how to update it")
        lines.append("   - Example: 'selectedFile = filePath' → const [selectedFile, setSelectedFile] = useState()")
        lines.append("")
        lines.append("6. LOADING, ERROR, AND EMPTY STATES:")
        lines.append("   - For onDataFetchStart → show loading skeleton")
        lines.append("   - For onDataFetchError → show error UI with retry button")
        lines.append("   - For onDataEmpty → show empty state message")
        lines.append("")
        lines.append("INTERACTION MODEL FROM UX DESIGNER:")
        lines.append("")

        # Format each interaction
        for i, interaction in enumerate(interaction_model, 1):
            event = interaction.get('event', 'unknown')
            lines.append(f"--- Interaction {i}: {event} ---")

            if interaction.get('triggers'):
                lines.append(f"  TRIGGERS: {interaction['triggers']}")

            state_changes = interaction.get('state_changes', [])
            if state_changes:
                lines.append(f"  STATE_CHANGES: {', '.join(state_changes)}")

            data_fetch = interaction.get('data_fetch', {})
            if data_fetch:
                endpoint = data_fetch.get('endpoint', '')
                params = data_fetch.get('params', [])
                trigger = data_fetch.get('trigger', '')
                if endpoint:
                    lines.append(f"  DATA_FETCH:")
            ...
            if ui_feedback:
                lines.append(f"  UI_FEEDBACK: {', '.join(ui_feedback[:4])}")

            dependency_rules = interaction.get('dependency_rules', [])
            if dependency_rules:
                lines.append(f"  DEPENDENCY_RULES: {', '.join(dependency_rules[:3])}")

            react_pattern = interaction.get('react_pattern', '')
            if react_pattern:
                lines.append(f"  REACT_PATTERN: {react_pattern}")

            lines.append("")

        # Add example implementation
        lines.append("EXAMPLE IMPLEMENTATION FOR onFileSelect:")
        lines.append("")
        lines.append("1. In dataHooks.tsx, add useFilePreview hook:")
        lines.append("```typescript")
        lines.append("export function useFilePreview(")
        lines.append("  pipelineId: string,")
        lines.append("  filePath: string | null,")
        lines.append("  opts: { page: number; pageSize: number }")
        lines.append(") {")
        lines.append("  const [data, setData] = useState<{ columns: string[]; rows: any[] } | null>(null);")
        lines.append("  const [loading, setLoading] = useState(false);")
        lines.append("  const [error, setError] = useState<string | null>(null);")
        lines.append("  const [totalRows, setTotalRows] = useState(0);")
        lines.append("")
        lines.append("  useEffect(() => {")
        lines.append("    if (!filePath) return;  // CRITICAL: Do NOT fetch if no file selected")
        lines.append("    setLoading(true);")
        lines.append("    setError(null);")
        lines.append("")
        lines.append("    // CORRECT ENDPOINT: /api/pipelines/{pipelineId}/files/preview")
        lines.append("    // CORRECT PARAMS: file_path, page, page_size")
        lines.append("    const url = `http://localhost:8000/api/pipelines/${encodeURIComponent(pipelineId)}/files/preview?file_path=${encodeURIComponent(filePath)}&page=${opts.page}&page_size=${opts.pageSize}`;")
        lines.append("")
        lines.append("    fetch(url)")
        lines.append("      .then(res => res.json())")
        lines.append("      .then(json => {")
        lines.append("        // RESPONSE SHAPE: { columns, rows, totalRows, page, pageSize }")
        lines.append("        setData({ columns: json.columns || [], rows: json.rows || [] });")
        lines.append("        setTotalRows(json.totalRows || 0);")
        lines.append("      })")
        lines.append("      .catch(err => setError(err.message))")
        lines.append("      .finally(() => setLoading(false));")
        lines.append("  }, [pipelineId, filePath, opts.page, opts.pageSize]); // DEPENDENCY RULES!")
        lines.append("")
        lines.append("  return { data, loading, error, totalRows };")
        lines.append("}")
        lines.append("```")
        lines.append("")
        lines.append("2. In DataPreviewTable.tsx, use the hook:")
        lines.append("```typescript")
        lines.append("export default function DataPreviewTable({ pipelineId, filePath, currentPage, rowsPerPage }) {")
        lines.append("  if (!filePath) {")
        lines.append("    return <div className='text-gray-500'>Select a file to preview</div>;")
        lines.append("  }")
        lines.append("")
        lines.append("  const { data, loading, error, totalRows } = useFilePreview(pipelineId, filePath, {")
        lines.append("    page: currentPage,")
        lines.append("    pageSize: rowsPerPage")
        lines.append("  });")
        lines.append("")
        lines.append("  if (loading) return <div className='animate-pulse'>Loading...</div>;")
        lines.append("  if (error) return <div className='text-red-500'>Error: {error}</div>;")
        lines.append("  if (!data || data.rows.length === 0) return <div>No data</div>;")
        lines.append("")
        lines.append("  return (")
        lines.append("    <table>")
        lines.append("      <thead><tr>{data.columns.map(c => <th key={c}>{c}</th>)}</tr></thead>")
        lines.append("      <tbody>{data.rows.map((r, i) => <tr key={i}>...</tr>)}</tbody>")
        lines.append("    </table>")
        lines.append("  );")
        lines.append("}")
        lines.append("```")
        lines.append("")
        lines.append("CRITICAL: The above pattern MUST be used for ANY component displaying remote data.")
        lines.append("Using hardcoded arrays is a VIOLATION of this specification.")
        lines.append("")
        lines.append("=" * 80)

        return "\n".join(lines)

    def _query_pinecone_patterns(self, design_spec: 'DesignSpec') -> str:
        """
        Query Pinecone for relevant interaction patterns based on design spec.

        This provides the React Developer with canonical patterns from the
        knowledge base, including correct endpoints, params, and anti-patterns.

        Args:
            design_spec: Design specification to extract query context from

        Returns:
            Formatted string with relevant patterns for the prompt
        """
        kb = _get_design_kb()
        if not kb:
            return ""

        # Build query from design spec components
        queries = []

        # Query based on component types in the spec
        if hasattr(design_spec, 'components') and design_spec.components:
            for comp in design_spec.components:
                comp_type = comp.get('type', '').lower()
                if 'table' in comp_type or 'preview' in comp_type:
                    queries.append("file preview fetch data table")
                if 'tree' in comp_type or 'explorer' in comp_type:
                    queries.append("file explorer drilldown selection")
                if 'pagination' in comp_type:
                    queries.append("pagination navigation page change")

        # Query based on interaction_model events
        if hasattr(design_spec, 'interaction_model') and design_spec.interaction_model:
            for interaction in design_spec.interaction_model:
                event = interaction.get('event', '')
                if event:
                    queries.append(f"event {event}")

        # Default query if nothing specific found
        if not queries:
            queries = ["data fetch file preview selection"]

        # Query Pinecone and collect unique patterns
        seen_ids = set()
        patterns = []
        for query in queries[:3]:  # Limit to 3 queries
            try:
                results = kb.query_interaction_patterns(query, top_k=2)
                ...
                print(f"[React Developer] Pinecone query failed: {e}")
        ...
        lines.append("")
        lines.append("=" * 80)
        lines.append("📚 CANONICAL INTERACTION PATTERNS FROM KNOWLEDGE BASE")
        lines.append("=" * 80)
        lines.append("")
        lines.append("These patterns are the AUTHORITATIVE source for endpoint URLs, params, and")
        lines.append("implementation details. ALWAYS follow these patterns exactly.")
        lines.append("")

        for pattern in patterns[:5]:  # Limit to 5 patterns
            lines.append(f"--- {pattern.get('name', 'Unknown')} (id: {pattern.get('id')}) ---")

            data_fetch = pattern.get('data_fetch', {})
            if data_fetch:
                endpoint = data_fetch.get('endpoint', '')
                params = data_fetch.get('params', [])
                query_format = data_fetch.get('query_string_format', '')
                response = data_fetch.get('response_shape', {})
                if endpoint:
                    lines.append(f"  ENDPOINT: {endpoint}")
                if params:
                    lines.append(f"  PARAMS: {', '.join(params)}")
                if query_format:
                    lines.append(f"  QUERY FORMAT: {query_format}")
                if response:
                    lines.append(f"  RESPONSE: {json.dumps(response)}")

            react_impl = pattern.get('react_implementation', {}) if isinstance(pattern.get('react_implementation'), dict) else {}
            url_template = react_impl.get('url_template', '') if isinstance(react_impl, dict) else ''
            if url_template:
                lines.append(f"  URL TEMPLATE: {url_template}")

            # CRITICAL: Include anti-patterns
            anti_patterns = react_impl.get('anti_patterns', []) if isinstance(react_impl, dict) else []
            if anti_patterns:
            ...
            if dependency_rules:
                lines.append(f"  DEPENDENCY RULES: {', '.join(dependency_rules[:3])}")
    ...
    def _validate_no_mock_data(self, files: Dict[str, str], raise_on_violation: bool = False):
        """
        ...
        Args:
            ...
            raise_on_violation: If True, raises ValueError on violation (use in autonomous mode)

        Raises:
            ValueError: If violations are detected and raise_on_violation=True
        """
        mock_patterns = [
            'MOCK_DATA',
            'MOCK_DATASETS',
            ...
            'const mockData =',
            'const MOCK_',
            # NOTE: 'const data = [' removed - too broad, catches legitimate patterns
            # Now using regex-based detection for more precision
        ]

        # More specific patterns using regex to avoid false positives
        import re
        mock_regex_patterns = [
            r'const \w+Data\s*=\s*\[',       # someData = [...] - hardcoded arrays
            r'const \w+Items\s*=\s*\[',      # someItems = [...] - hardcoded arrays
            r'useState\(\s*\[\s*\{',         # useState([{...}]) - inline mock state
            r'const \w+\s*=\s*\[\s*\{\s*id:',  # Hardcoded array of objects with id
        ]
        ...
        for filename, content in files.items():
            ...
            # Check for mock data patterns (string-based)
            ...
            # Check for mock data patterns (regex-based, more precise)
            for regex_pattern in mock_regex_patterns:
                for i, line in enumerate(content.split('\n')):
                    ...
                    if re.search(regex_pattern, line):
                        violations.append(f"{filename}:{i+1}: Regex match '{regex_pattern}' on: {stripped[:80]}")
...
"""
            print(warning)
            # In autonomous mode, fail hard to prevent mock data from slipping through
            if raise_on_violation:
                raise ValueError(f"Mock data violation detected:\n{violation_msg}")
    ...
    def _validate_type_imports(self, files: Dict[str, str]):
        """
        FIX RC-3: Validate that imported types actually exist in types.ts.

        Detects imports of non-existent types like:
        - import { Stage } from '../types';      // Stage doesn't exist
        - import { FilePreviewData } from '...'; // FilePreviewData doesn't exist

        Args:
            files: Dict of filename -> content
        """
        # Canonical types that exist in types.ts / dataHooks.tsx
        VALID_TYPES = {
            'Pipeline', 'PipelineStage', 'FileNode', 'PipelinesResponse',
            'FilePreviewResponse', 'PipelineMetrics'
        }

        # Pattern to find type imports from types.ts or dataHooks.tsx
        import_pattern = re.compile(
            r"import\s+(?:type\s+)?{\s*([^}]+)\s*}\s+from\s+['\"]\.\.?/(?:types|dataHooks)(?:\.tsx?)?['\"]"
        )
        ...
        for filename, content in files.items():
            if not filename.endswith(('.tsx', '.ts')):
                continue
            if 'types.ts' in filename or 'dataHooks.tsx' in filename:
                continue

            for match in import_pattern.finditer(content):
                imported_items = match.group(1)
                # Parse individual imports (handle "Type1, Type2 as Alias")
                for item in imported_items.split(','):
                    ...
                    if type_name and type_name not in VALID_TYPES:
                        warnings.append(f"{filename}: Imports non-existent type '{type_name}'")

        if warnings:
            print("\n" + "="*70)
            print("WARNING: NON-EXISTENT TYPE IMPORTS DETECTED (RC-3)")
            print("="*70)
            print("\nThese types don't exist in types.ts:")
    ...
    def _parse_generated_files(self, generated_code: str) -> Dict[str, str]:
        ...
        # Validate type imports - NO HALLUCINATED TYPES (RC-3)
        self._validate_type_imports(files)
...
"""
    ...
    def save_files(self, files: Dict[str, str], output_dir: Path):
        ...
        # FIX RC-5: Clean up duplicate src/src/ directory from previous runs
        src_src_dir = output_dir / 'src' / 'src'
        if src_src_dir.exists():
            import shutil
            shutil.rmtree(src_src_dir)
            print(f"  [React Developer] Cleaned up duplicate directory: {src_src_dir}")
        ...
        for filename, content in files.items():
            ...
            if 'components/' in filename or is_source_file:
                ...
                # Fix: Strip leading src/ if already present to avoid src/src/ duplication
                clean_filename = filename
                if filename.startswith('src/'):
                    clean_filename = filename[4:]  # Remove 'src/' prefix
                file_path = output_dir / 'src' / clean_filename
            else:
        ...
        # Fix vite.config.ts to use port 3001 to avoid conflict with other services
        ...
        if vite_config_path.exists():
            ...
            if 'server:' not in vite_content:
                # Inject server config block - handle both with and without trailing comma after plugins
                if 'plugins: [react()],' in vite_content:
  ...
  server: {
    ...
    strictPort: true,
                ...
                elif 'plugins: [react()]' in vite_content:
                    vite_content = vite_content.replace(
                        'plugins: [react()]',
                        '''plugins: [react()],
  server: {
    ...
    strictPort: true,
                ...
                print(f"  [React Developer] Injected server config with port 3001")
            else:
                # Server config exists, just fix the port
                vite_content = vite_content.replace('port: 3000', 'port: 3001')
                vite_content = vite_content.replace('port: 5173', 'port: 3001')
